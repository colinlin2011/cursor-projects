...
PARENT_NODE_TOKEN = "C6ixwp4nxi5I4qkDvTjcWKIhn0e"  # 指定的父节点
DOC_TITLE = "舱驾一体域控的TSC"

# 详细的TSC内容
TSC_CONTENT = """# 舱驾一体域控的TSC
...
def main():
    ...
    print(f"[OK] 文档节点token: {doc_info['node_token']}")
    print(f"[OK] 文档ID: {doc_info['document_id']}")
    print(f"[OK] 文档链接: {doc_info['doc_url']}")
    print()
    
    # 获取本地Markdown文件路径
    md_path = collaborator.get_local_md_path(DOC_TITLE)
    print(f"步骤2：创建本地Markdown文件...")
    ...
    print(f"[OK] Markdown文件已创建")
    print()
    
    # 同步到飞书
    print("步骤3：同步Markdown内容到飞书文档...")
    ...
    if success:
        ...
        print()
        print(f"本地Markdown文件: {md_path}")
        print()
        print("提示：")
        print("1. 可以在本地Markdown文件中继续编辑")
        print("2. 编辑完成后，运行以下命令同步到飞书：")
        print(f'   python quick_doc.py sync-to "{doc_info["node_token"]}" --title "{DOC_TITLE}"')
    else:
        print("[X] 同步失败")
