...
for i, record in enumerate(records[:10]):
    ...
    if function_description:
        func_type = type(function_description).__name__
        if isinstance(function_description, list):
            print(f"\n记录{i+1} - 功能描述是列表格式（富文本）:")
        ...
        elif isinstance(function_description, str):
            print(f"记录{i+1} - 功能描述是字符串: {function_description[:60]}")
